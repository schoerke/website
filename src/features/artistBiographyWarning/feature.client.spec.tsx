...
import { ArtistBiographyWarningFeatureClient } from './feature.client'
...
describe('ArtistBiographyWarningFeatureClient', () => {
  it('shows a localized warning list and toggles field class from editor updates', async () => {
    ...
    const feature = ArtistBiographyWarningFeatureClient as unknown as { plugins: [{ Component: React.FC }] }
    const Plugin = feature.plugins[0].Component
    const view = render(<Plugin />)

    const alert = screen.getByRole('alert')
    expect(alert).toHaveTextContent('Die Biographie darf keine Bilder oder eingebetteten Medien enthalten.')
    expect(alert).toHaveTextContent('Leere Zeilen am Ende entfernen.')
    expect(alert).toHaveTextContent('Bitte mit einem Satz beginnen, der als Kurzbeschreibung geeignet ist.')
    expect(screen.getAllByRole('listitem')).toHaveLength(3)
    expect(field).toHaveClass('artist-biography-warning')

    act(() => harness.listener?.({ editorState: { toJSON: () => validContent } }))

    await waitFor(() => expect(screen.queryByRole('alert')).not.toBeInTheDocument())
    expect(field).not.toHaveClass('artist-biography-warning')
  ...
  it('renders English messages for an English locale', async () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('The biography cannot contain images or embedded media.')
    expect(screen.getByRole('alert')).toHaveTextContent('Remove any empty lines at the end.')

    view.unmount()
  })

  it('shows no banner for valid PerformersList content', () => {
    const field = document.createElement('div')
    field.className = 'field-type rich-text-lexical'
    field.append(harness.root)
    document.body.append(field)
        ...
        toJSON: () => ({
          root: {
            children: [
              ...
              { type: 'block', fields: { blockType: 'performersList' } },
            ],
          },
        }),
      }),
