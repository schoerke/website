...
generated migration via a new `ci` script (`pnpm migrate && pnpm build`) wired into Vercel.
...
| `vercel.json` | **Create** — `buildCommand: pnpm ci` |
...
## Task 11: ci script + vercel.json

**Files:**
- Modify: `package.json`
- Create: `vercel.json`

- [ ] **Step 1: Add the `ci` script**
...
Run: `pnpm build`
Expected: builds successfully. `migrate` is NOT run locally — only `pnpm ci` runs it (used by Vercel).

- [ ] **Step 4: Commit**

```bash
git add package.json vercel.json
git commit -m "build: add ci script and vercel buildCommand for migrations"
```
...
be applied to prod by the Vercel `ci` build on next deploy.
...
1. `pnpm ci` runs → `pnpm migrate` applies `artist_repertoire_ordering` to the **prod** DB
...
**DB protection policy:** applying the migration to prod (via deploy) and running the backfill against prod both
...
- **Build:** revert `vercel.json` and the `ci` script to restore previous build behavior.
